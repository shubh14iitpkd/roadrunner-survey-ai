...
class SageMakerVideoProcessor:
    ...
    def _check_gpu_availability(self) -> bool:
        ...
        try:
            ...
            code = subprocess.check_call(['nvidia-smi'], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
    ...
    def get_ffmpeg_command(self, width: int, height: int, fps: int, output_path: Path) -> List[str]:
        """
                ...
                for chunk_start in range(0, total_frames, chunk_size):
                    ...
                    for frame_num in range(chunk_start, chunk_end):
                        ...
                        if frame_num % self.frame_interval == 0:
                            future = executor.submit(self._process_single_frame, frame, width, height, frame_num)
                    ...
                    for future in as_completed(futures):
                        ...
                        try:
                            ...
                            single_frame_results = (None,chunk_frames[chunk_idx][1],[])
                    ...
                    for chunk_idx, (frame_num, frame, timestamp) in enumerate(chunk_frames):
                        if chunk_idx in inference_results:
                            ...
                            _, annotated_frame, detections = inference_results[chunk_idx]
                            ...
                            # }
                            frame_metadata.append({ "frame_num": frame_num, "timestamp" : timestamp, "detections": detections })
    ...
    def _invoke_sagemaker(self, frame: np.ndarray, orig_width: int, orig_height: int) -> List[Dict]:
        ...
        try:
            if not self.endpoint_name or self.endpoint_name.lower() == 'mock' or not self.sagemaker_runtime: 
