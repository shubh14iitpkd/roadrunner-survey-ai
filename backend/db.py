...
def init_app_db(app: Flask) -> None:
	# Temp client for index setup — closed before fork so workers open own pools
	tmp_client = MongoClient(app.config["MONGO_URI"], uuidRepresentation="standard")
	try:
		db = tmp_client[app.config["MONGO_DB_NAME"]]
		# Users
		try:
