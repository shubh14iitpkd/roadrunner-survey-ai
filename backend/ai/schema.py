...
SCHEMA = {
    "frames": {
        "description": "Video frames extracted from road survey videos with computer vision detections. CRITICAL: 'detections' field contains ALL detected objects (street lights, signs, road markings, etc.) - these ARE the defects/items of interest. When user asks 'how many defects', COUNT ALL DETECTIONS without filtering by class_name.",
        "collection_name": "frames",
        "priority": "high",
        "key_fields": ["video_id", "route_id", "frame_number", "timestamp", "location"],
        "primary_intent": "Answer questions about what is visible on the road, object detection results, infrastructure presence, defect analysis, frame-level evidence, and spatial positioning. IMPORTANT: 'Defects' means ALL detections.",
        "when_to_use": [
            ...
            "User asks 'how many defects' → COUNT ALL DETECTIONS (no class_name filter!)",
        ...
        "fields": {
            ...
            "video_id": "String - Reference to parent video (HIGH priority for video-level questions)",
            "survey_id": "ObjectId - Reference to survey session",
            "route_id": "Integer - Road route number (HIGHEST priority for route-based queries)",
            "frame_number": "Integer - Sequential frame number in video",
            "timestamp": "Float - Time in video (seconds) for temporal navigation",
            "frame_path": "String - Path to frame image file (for visual evidence)",
            "detections": "Array - All detected objects in frame (HIGHEST priority)",
            "detections[].detection_id": "Integer - Detection index within frame",
            "detections[].class_id": "Integer - Class identifier",
            "detections[].class_name": "String - Detection class (Street lights, Road markings, etc)",
            "detections[].confidence": "Float - Model confidence score (0-1)",
            "detections[].bbox": "Object - Bounding box {x1, y1, x2, y2}",
            "detections_count": "Integer - Number of detections in frame",
            "location": "GeoJSON - {type: 'Point', coordinates: [lng, lat]} (HIGHEST priority for spatial search)",
