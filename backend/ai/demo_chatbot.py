...
class DemoDataLoader:
    ...
    def get_assets_by_type(self, asset_type: str, video_id: str = None) -> List[Dict]:
        ...
        for a in self.assets:
            ...
            # Check all variants
            if (type_lower in asset_type_val or type_underscore in asset_type_val or 
                type_space in asset_type_val or type_lower in class_name_val or
                type_underscore in class_name_val or type_space in class_name_val):
    ...
    def get_condition_breakdown_for_type(self, asset_type: str, video_id: str = None) -> Dict:
        ...
        for a in assets:
            ...
            if condition in ['good', 'fine', 'visible']:
                good_count += 1
            elif condition in ['damaged', 'bad', 'poor', 'missing', 'broken', 'bent', 'dirty', 'overgrown']:
                damaged_count += 1
            else:
    ...
    def get_damaged_assets_for_improvements(self, video_id: str = None) -> List[Dict]:
        ...
        for a in assets:
            condition = a.get('condition', '').lower()
            if condition in ['damaged', 'bad', 'poor', 'missing', 'broken', 'bent', 'dirty', 'overgrown']:
        ...
        result = []
        for asset_type, data in sorted(damaged_by_type.items(), key=lambda x: -x[1]['count']):
            result.append({
                'type': asset_type,
                'count': data['count'],
...
class FramesDBClient:
    ...
    def get_frame_at_timestamp(self, key: str, timestamp_seconds: float, tolerance: float = 1.0) -> List[Dict]:
...
class DemoChatbot:
    ...
    def _detect_query_type(self, question: str) -> str:
        ...
        if re.search(r'(?:at|timestamp)\s*\d+[:\s]\d{2}', q_lower) or \
           re.search(r'(?:at|timestamp)\s*\d+\s*(?:seconds?|sec|s)\b', q_lower):
            return 'timestamp'
    ...
    def ask(self, question: str, video_id: str = None) -> str:
        ...
        if query_type == 'timestamp':
            response = self._handle_timestamp_query(question, video_key, total_frames)
        elif query_type == 'frame':
            response = self._handle_frame_query(question, video_key, total_frames)
        else:
            response = self._handle_asset_query(question, video_key, total_frames)
    ...
    def _handle_timestamp_query(self, question: str, video_key: str, total_frames: int) -> str:
        ...
        # Build response
        lines = [f"**At {self._format_timestamp(timestamp)}** (Video: {video_key}):\n"]
        ...
        for frame in frames:
            ...
            if detections:
                ...
                lines.append(f"• **Frame {frame_num}** ({self._format_timestamp(frame_ts)}): {det_str}")
            else:
                lines.append(f"• **Frame {frame_num}** ({self._format_timestamp(frame_ts)}): No detections")
    ...
    def _handle_frame_query(self, question: str, video_key: str, total_frames: int) -> str:
        ...
        # Build response
        lines = [f"**Frame {frame_num}** (Video: {video_key}):\n"]
    ...
    def _handle_asset_query(self, question: str, video_key: str, total_frames: int) -> str:
        ...
        if intent.get('wants_defect_analysis'):
            return self._handle_defect_analysis(video_key)
        
        if intent.get('wants_improvements'):
            return self._handle_improvements_query(video_key)
        
        if intent.get('wants_condition_breakdown') and intent.get('asset_type'):
            return self._handle_condition_breakdown(intent['asset_type'], video_key)
        
        # Default asset query handling
        context = self._get_context_data(intent, video_key)
        context['video_name'] = video_key or 'All videos'
        
        if self.client:
            return self._generate_answer_with_gemini(question, context, intent, video_key)
        else:
            return self._generate_simple_answer(question, context, intent)
    
    def _handle_defect_analysis(self, video_key: str) -> str:
        ...
        lines = [f"**Defect Analysis** - {video_key or 'All videos'}\n"]
        lines.append(f"Total damaged assets: **{total_defects:,}**\n")
        lines.append(f"**{worst_category[0]}** has the highest number of defects with **{worst_category[1]:,}** damaged items.\n")
    ...
    def _handle_condition_breakdown(self, asset_type: str, video_key: str) -> str:
    ...
    def _handle_improvements_query(self, video_key: str) -> str:
        ...
        lines = [f"To improve the road shown in {video_key or 'All videos'}\n"]
        lines.append(f"Found **{total_damaged:,}** damaged assets requiring attention.\n")
        ...
        for item in damaged:
            ...
            suggestion = suggestions.get(asset_type, f'Inspect and repair damaged {asset_type.replace("_", " ").lower()}')
    ...
    def _analyze_intent(self, question: str) -> Dict:
        ...
        if any(phrase in q_lower for phrase in ['most defect', 'most damage', 'highest defect', 
                                                  'category.*defect', 'defect.*category',
                                                  'which.*damage', 'what.*damage']):
            intent['wants_defect_analysis'] = True
        
        # Improvement suggestions: "improve", "fix", "repair", "what can be done"
        if any(phrase in q_lower for phrase in ['improve', 'fix', 'repair', 'maintenance',
        ...
        if any(phrase in q_lower for phrase in ['condition of', 'condition for', 
                                                  'state of', 'health of']):
        ...
        # Summary queries
        if any(word in q_lower for word in ['summary', 'summarize', 'overview', 'breakdown']):
        ...
        asset_types = [
            'street light', 'traffic sign', 'traffic signal', 'kerb', 'road marking',
            'guardrail', 'fence', 'bollard', 'flyover', 'cctv', 'median', 'shoulder',
            'carriageway', 'footpath', 'parking bay', 'speed hump', 'rumble strip',
            'overpass', 'underpass', 'bridge', 'pole', 'emergency phone', 'fire extinguisher',
            'traffic bollard', 'street sign', 'directional sign'
        ]
    ...
    def _generate_answer_with_gemini(self, question: str, context: Dict, 
                                      intent: Dict, video_key: str = None) -> str:
        """Generate natural language answer using Gemini"""
        
        video_info = f"Video: {video_key}" if video_key else "All demo videos"
...
"""
        ...
        if context.get('filtered_count') is not None:
            filter_desc = context.get('filter_type') or context.get('filter_condition') or context.get('filter_category', 'filter')
            data_context += f"\nFILTERED ({filter_desc}): {context['filtered_count']} assets"
        
        prompt = f"""You are a road survey assistant. Answer the user's question based on the data provided.
...
3. Format numbers with commas (1,234)
4. Never show database IDs - use friendly names
...
Answer:"""
    ...
    def _generate_simple_answer(self, question: str, context: Dict, intent: Dict) -> str:
        ...
        if context.get('filtered_count') is not None:
            filter_desc = context.get('filter_type') or context.get('filter_condition') or context.get('filter_category', '')
